        ...
        async with session.get(BASE_URL, headers=HEADERS) as response:
            ...
            for link in links:
                ...
                async with session.get(URL_EQUITIES, headers=HEADERS) as response:
                    ...
                    price = variableSITE.find("div", {"class": "text-5xl/9 font-bold text-[#232526] md:text-[42px] md:leading-[60px]"}).text
                    title = variableSITE.find("h1", {"class": "mb-2.5 text-left text-xl font-bold leading-7 text-[#232526] md:mb-2 md:text-3xl md:leading-8 rtl:soft-ltr"}).text
